# Prompt Hari 27 — Cart merge, revalidation, voucher, dan save-for-later
...
- docs/14-ai-build-playbook/progress/DAY-26-REPORT.md (bila Day > 01 dan file tersedia)

SCOPE HARI INI, URUT SESUAI BLOK KERJA:
1. 08:00–10:00 — Implement POST /cart/merge transaction: merge variant sama, clamp stock/purchase limit, latest price, merged marker, dan idempotency.
2. 10:15–12:15 — Implement revalidation saat cart dibaca/diubah, warning changes, inactive cleanup policy, dan race-safe versioning.
3. 13:15–15:15 — Implement save-for-later dan voucher validation/application dasar dengan explicit rule engine serta integer amounts.
4. 15:30–17:30 — Tulis merge matrix tests, price/stock changes, duplicate retry, voucher edge cases, dan rollback behavior.
...
2. File `docs/14-ai-build-playbook/progress/DAY-27-REPORT.md` mengikuti template laporan harian.
3. Laporan harus memuat: ringkasan, file berubah, keputusan, migration, env baru, command yang dijalankan beserta PASS/FAIL, acceptance evidence, security review, blocker, technical debt, dan handoff Day 28.
