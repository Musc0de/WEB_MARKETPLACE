# Prompt Hari 31 — Transactional outbox worker, locking, retry, dan cleanup jobs
...
- docs/14-ai-build-playbook/progress/DAY-30-REPORT.md (bila Day > 01 dan file tersedia)

SCOPE HARI INI, URUT SESUAI BLOK KERJA:
1. 08:00–10:00 — Implement outbox poller dengan claim/lock aman, batch size, available_at, SKIP LOCKED atau strategi setara, correlation ID, dan graceful shutdown.
2. 10:15–12:15 — Implement handler registry, idempotency/dedupe, exponential backoff with cap/jitter, max attempts, failed state/dead-letter semantics.
3. 13:15–15:15 — Implement scheduled cleanup untuk expired session/token, release expired inventory reservation, dan stale guest cart policy.
4. 15:30–17:30 — Tulis worker concurrency/retry/crash-recovery tests serta operational commands untuk inspect/retry failed job.
...
2. File `docs/14-ai-build-playbook/progress/DAY-31-REPORT.md` mengikuti template laporan harian.
3. Laporan harus memuat: ringkasan, file berubah, keputusan, migration, env baru, command yang dijalankan beserta PASS/FAIL, acceptance evidence, security review, blocker, technical debt, dan handoff Day 32.
