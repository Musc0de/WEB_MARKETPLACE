...
function StatusBadge({ status }: { status: string }) {
  const cfg = STATUS_BADGE[status] ??
    { label: status.toUpperCase(), className: 'bg-gray-100 text-gray-700' };
  return (
    <span
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='grid grid-cols-2 gap-4 sm:grid-cols-4'>
        ...
        <SummaryCard
          icon={CheckCircle2}
          label='Avg per Pesanan'
          value={formatIDR(summary.totalOrders ? (summary.totalRevenue / summary.totalOrders) : 0)}
          color='purple'
        />
        <SummaryCard
          icon={Clock}
      ...
      <div className='flex justify-end'>
        <button
          type='button'
          onClick={() => exportCSV(rows, 'laporan_penjualan')}
    ...
    <div className='space-y-4'>
      <div className='grid grid-cols-2 gap-4 sm:grid-cols-4'>
        <SummaryCard
          icon={TrendingUp}
          label='Gross Revenue'
          value={formatIDR(summary.grossRevenue ?? 0)}
          color='green'
        />
        <SummaryCard
          icon={CheckCircle2}
        ...
        <SummaryCard
          icon={RotateCcw}
          label='Direfund'
          value={String(summary.refundedOrders ?? 0)}
          color='purple'
        />
      </div>
      ...
      <div className='flex justify-end'>
        <button
          type='button'
          onClick={() => exportCSV(rows, 'laporan_keuangan')}
          className='inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-600 shadow-sm hover:bg-gray-50'
        >
          <Download className='h-4 w-4' /> Export CSV
        </button>
    ...
    <div className='space-y-4'>
      <div className='grid grid-cols-2 gap-4 sm:grid-cols-3'>
        <SummaryCard
          ...
          value={String(summary.totalAvailable ?? 0)}
          sub='Unit tersedia di semua gudang'
          color='green'
        />
        <SummaryCard
          icon={Clock}
          label='Total Direservasi'
          value={String(summary.totalReserved ?? 0)}
          sub='Dikunci oleh pesanan aktif'
          color='amber'
        />
        <SummaryCard
          icon={AlertTriangle}
          label='Total Rusak/Hilang'
          value={String(summary.totalDamaged ?? 0)}
          sub='Unit dalam kondisi rusak'
          color='red'
        />
      </div>
      ...
      <div className='overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm'>
        <table className='min-w-full divide-y divide-gray-200'>
          ...
          <tbody className='divide-y divide-gray-100'>
            {isLoading ? <TableSkeleton cols={7} /> : rows.length === 0
              ? (
                <tr>
                  ...
                  <td className='px-6 py-4'>
                    <span
                      className={`text-sm font-bold ${
                        row.available <= 5 ? 'text-red-600' : 'text-emerald-600'
                      }`}
                    >
                      {row.available}
                    </span>
      ...
      <div className='flex justify-end'>
        <button
          type='button'
          onClick={() => exportCSV(rows, 'laporan_stok')}
          className='inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-600 shadow-sm hover:bg-gray-50'
        >
          <Download className='h-4 w-4' /> Export CSV
        </button>
    ...
    <div className='space-y-4'>
      <div className='grid grid-cols-2 gap-4 sm:grid-cols-2'>
        <SummaryCard
          ...
          sub='Permintaan retur dalam periode ini'
          color='purple'
        />
        <SummaryCard
          icon={AlertTriangle}
          label='Total Stok Rusak'
          value={String(summary.totalDamagedStock ?? 0)}
          sub='Unit rusak di seluruh gudang'
          color='red'
        />
      </div>
      ...
      <div className='overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm'>
        <table className='min-w-full divide-y divide-gray-200'>
          <thead className='bg-gray-50'>
            <tr>
              {['ID Retur', 'Order ID', 'Alasan', 'Resolusi', 'Jml Item', 'Status', 'Tanggal'].map((
                h,
              ) => (
                <th
                  key={h}
      ...
      <div className='flex justify-end'>
        <button
          type='button'
          onClick={() => exportCSV(rows, 'laporan_retur')}
          className='inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-600 shadow-sm hover:bg-gray-50'
        >
          <Download className='h-4 w-4' /> Export CSV
        </button>
...
const TABS: { key: ReportTab; label: string; icon: typeof BarChart3; description: string }[] = [
  { key: 'sales', label: 'Penjualan', icon: TrendingUp, description: 'Semua transaksi pesanan' },
  {
    ...
    label: 'Keuangan',
    icon: BarChart3,
    description: 'Revenue & transaksi sukses',
  },
  { key: 'stock', label: 'Stok', icon: Package, description: 'Kondisi inventaris produk' },
  {
    key: 'returns',
    label: 'Retur & Kehilangan',
    icon: RotateCcw,
    description: 'Pengembalian & kerusakan barang',
  },
];
