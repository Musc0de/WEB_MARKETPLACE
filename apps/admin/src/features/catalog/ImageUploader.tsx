...
export function ImageUploader(
  { productId: _productId, productName, onUploadSuccess }: ImageUploaderProps,
) {
    ...
    <div className='space-y-6'>
        ...
        <div className='space-y-2'>
          ...
          <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4'>
            {existingImages.map((img) => (
              <div
                key={img.id}
                className='relative group aspect-square rounded-lg overflow-hidden bg-gray-100 border border-gray-200 shadow-sm'
              >
                <img
                  src={img.url}
                  alt={img.objectKey}
      ...
      >
        {files.length > 0
          ...
          : (
            <div
              className='flex flex-col items-center justify-center gap-3 py-10 text-center px-4'
              style={{ minHeight: 180 }}
            >
              ...
              <div>
                ...
                <p className='text-xs text-gray-400 mt-0.5'>
                  JPG, PNG, WebP — maks. {MAX_MB}MB — maksimal {MAX_FILES} file sekaligus
                </p>
              </div>
            </div>
          )}
