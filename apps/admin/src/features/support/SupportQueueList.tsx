...
import { HeadphonesIcon, ArrowRight, Clock, AlertTriangle, CheckCircle2 } from 'lucide-react';
import {
  PageHeader,
  SearchBar,
  StatusPill,
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-2 gap-3 sm:grid-cols-4'>
        {[
          { label: 'Total Tiket', value: allTickets.length, icon: HeadphonesIcon, color: 'text-gray-900', iconColor: 'text-blue-600', bg: 'bg-blue-50' },
          { label: 'Terbuka', value: openCount, icon: Clock, color: 'text-blue-700', iconColor: 'text-blue-600', bg: 'bg-blue-50' },
          { label: 'Diproses', value: inProgressCount, icon: CheckCircle2, color: 'text-amber-700', iconColor: 'text-amber-600', bg: 'bg-amber-50' },
          { label: 'Darurat', value: urgentCount, icon: AlertTriangle, color: 'text-red-700', iconColor: 'text-red-600', bg: 'bg-red-50' },
        ].map(({ label, value, icon: Icon, color, iconColor, bg }) => (
          <div key={label} className='flex items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-sm'>
          ...
          filtered.map((ticket: any) => (
            <tr key={ticket.id} className='hover:bg-rose-50/20 transition-colors'>
              ...
              <td className='px-5 py-3.5 max-w-xs'>
                <Link to={`/support/${ticket.id}`} className='text-sm font-medium text-gray-900 hover:text-blue-600 line-clamp-1 hover:underline'>
                  {ticket.subject}
                </Link>
              </td>
              <td className='px-5 py-3.5'>
              ...
              <td className='px-5 py-3.5'>
                <span className='inline-flex items-center gap-1 text-xs font-semibold capitalize text-gray-700'>
                  {PRIORITY_ICON[ticket.priority] ?? <Clock className='h-3.5 w-3.5 text-gray-400' />}
                  {ticket.priority}
                </span>
              </td>
              <td className='px-5 py-3.5'><StatusPill status={ticket.status} /></td>
              <td className='px-5 py-3.5 text-sm text-gray-400 whitespace-nowrap'>{formatDate(ticket.createdAt)}</td>
              <td className='px-5 py-3.5'>
                <Link
                  to={`/support/${ticket.id}`}
