...
export async function generateInvoicePdf(
  ...
  page.drawText(`StarSuperScare Marketplace`, {
    x: 50,
    y: height - 70,
    size: 12,
    font,
  ...
  cursorY -= 15;
  page.drawText(`${orderData.emailSnapshot || 'Customer'}`, {
    x: 50,
    y: cursorY,
    size: 11,
    font,
  ...
  page.drawText(`${formatter.format(orderData.subtotalAmount || 0)}`, {
    x: 450,
    y: cursorY,
    size: 10,
    font,
  ...
  page.drawText(`${formatter.format(orderData.shippingAmount || 0)}`, {
    x: 450,
    y: cursorY,
    size: 10,
    font,
  ...
  page.drawText(`${formatter.format(orderData.taxAmount || 0)}`, {
    x: 450,
    y: cursorY,
    size: 10,
    font,
  ...
  // Total Line
  page.drawLine({
    start: { x: totalsStartX, y: cursorY + 12 },
    end: { x: width - 50, y: cursorY + 12 },
    thickness: 1,
    color: blackColor,
  });

  page.drawText(`Grand Total:`, { x: totalsStartX, y: cursorY, size: 12, font: boldFont });
  page.drawText(`${formatter.format(orderData.totalAmount || 0)}`, {
    x: 450,
    y: cursorY,
    size: 12,
    font: boldFont,
