...
app.post(
  ...
  async (c) => {
    ...
    if (existing.length > 0) throw new HTTPException(409, { message: 'Brand slug already exists' });
...
app.delete('/:id', requirePermission('catalog.write'), async (c) => {
  ...
  await db.transaction(async (tx) => {
    await tx.update(brands).set({ deletedAt: new Date().toISOString() }).where(eq(brands.id, id));
    await logAudit(tx, {
      actorId: user.id,
      entityType: 'brand',
      entityId: id,
